...
public abstract class SuperSmoothMover extends Actor
{
    ...
    public void setLocation(double x, double y) {
        exactX = x;
        exactY = y;
        super.setLocation(neutralRound(x), neutralRound(y));
    }

    private int neutralRound (double value){
        return (int)( value + ((0.5) * Math.signum(value))); 
    }

    /**
    public void move(double distance) {
    // Pre-calculate movement with high precision
    double dx = Math.round(cosRotation * distance * 1000000.0) / 1000000.0;
    double dy = Math.round(sinRotation * distance * 1000000.0) / 1000000.0;
    setLocation(exactX + dx, exactY + dy);
    }
     */
    /**
